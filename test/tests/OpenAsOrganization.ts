  ...
  describe('OpenAsOrganization', ()=>{    
    before('write user to organization data', async () => {
      await th.txSuccess(
        Transactions.data(
          {
            data: [{key: `user_${th.OrganizationUser.address}`, value: Active}],
            chainId: th.chainId,
            fee: 500000,
          },
    ...
    describe('not allowed organization', ()=>{
      ...
      it('invoke', async ()=>{
        await th.txDappFail(Transactions.invokeScript({
          ...
          payment: [],
          fee: 900000 
        },invoker.seed),'Organization not permitted')
      })
      ...
      it('device not opened', async ()=>{
        ...
        expect(await th.dappValueFor(`device_counter_${device}`)).to.eq(th.deviceActionsCount)
      })
    })   
    ...
    describe('allowed organization', ()=>{
      ...
      describe('not organization user', ()=>{
        ...
        it('invoke', async ()=>{
          await th.txDappFail(Transactions.invokeScript({
            ...
            payment: [],
            fee: 900000 
          },invoker.seed),'Not permitted by organization')
        })
      ...
      describe('organization user', ()=>{
        ...
        it('device not opened', async ()=>{
          ...
          expect(await th.dappValueFor(`device_counter_${device}`)).to.eq(th.deviceActionsCount + 1)
        })
      })   
