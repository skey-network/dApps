  ...
  describe('OwnerRequestsKey', ()=>{
    before ('setup', async ()=>{
      ...
      const params = {
        ...
        fee: 1000000,
        transfers: [
            {amount: 100000000, recipient: th.DevOwner.address},
          ]
      } 
    ...
    describe('wrong payment amounts', function(){
      ...
      it('invoke', async ()=>{
        ...
        await th.txDappFail(Transactions.invokeScript({
          ...
          chainId: th.chainId,
          call: th.requestKeyCall,
          payment: [{amount: th.keyPrice * th.keyDuration - 5, assetId: null}],
          fee: 500000
           
      },th.DevOwner.seed), 'wrong payment value, expected '+th.keyPrice * th.keyDuration)
      })
      ...
      it('no key transfered', async ()=>{
        expect((await th.getNftFrom(th.DevOwner, th.Dapp)).length).to.eq(nftCount)
      })
    })
    ...
    describe('not owned device', function(){
      ...
      it('invoke', async ()=>{
        ...
        await th.txDappFail(Transactions.invokeScript({
          ...
          chainId: th.chainId,
          call: th.requestKeyCall,
          payment: [{amount: th.keyPrice * th.keyDuration, assetId: null}],
          fee: 500000
    ...
    describe('request key', function(){
      ...
      it('asset change', async ()=>{
        await th.balance.expectChange(th.DevOwner,-th.keyPrice * th.keyDuration - 500000 /*fee*/)    
      })
