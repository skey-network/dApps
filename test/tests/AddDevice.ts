  ...
  describe('AddDevice', ()=>{    
    before('write device data', async () => {
      await th.txSuccess(
        Transactions.data(
          {
            data: [{key: "owner", value: th.DevOwner.address},
    ...
    describe('not a dapp', ()=>{
      ...
      it('device not added', async ()=>{
        expect(await th.dappValueFor(`device_${th.Device.address}`)).to.eq(undefined)
        expect(await th.dappValueFor(`device_counter_${th.Device.address}`)).to.eq(undefined)
      })
    })   
   ...
   describe('same device second time', ()=>{
      it('invoke', async ()=>{
        await th.txDappFail(Transactions.invokeScript({
          ...
          payment: [],
          fee: 900000 
      },th.Dapp.seed),'Device already added')
      })
    }) 
  })
}
