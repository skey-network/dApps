...
import { IInvokeScriptCallStringArgument, TInvokeScriptCallArgument } from '@waves/waves-transactions/dist/transactions'
  ...
  describe('DappAddsManyKeysToDevice', ()=>{
    before('Generate list', async ()=>{
      ...
      const args : TInvokeScriptCallArgument[]  = [
          { type: "list", value: th.keysForTest.map((x)=>{
            return {type: "string", value: x} as IInvokeScriptCallStringArgument
          })},
        ]
    ...
    describe('random user tries to add keys', function(){
      ...
      it('keys not added', async ()=>{
        for(let i = 0; i<80; i++){
          expect(await th.walletValueFor(th.Device,`key_${th.keysForTest[i]}`)).to.eq(undefined)
        }
      })
    })
    ...
    describe('add keys', function(){
      ...
      it('keys added', async ()=>{
        for(let i = 0; i<80; i++){
          expect(await th.walletValueFor(th.Device,`key_${th.keysForTest[i]}`)).to.eq(ACTIVE)
        }
      })
    })
