...
async function deployDapp(seed: String) {
  ...
  const { stdout } = await execFile(
    'surfboard',
    ['run', 'scripts/wallet.deploy.js', '--variables', `SEED=${seed}`],
    { cwd: dir }
  )
  return stdout
}
...
async function txFail(signedTx, expectedMessage) {
...
async function fetchAssetBalance(account: Account, asset: string) {
  let resp = await fetch(
    `${chosenUrl}assets/balance/${account.address}/${asset}`
  )
...
async function setAssetActualBalance(account: Account, asset: string) {
  balances[`${account.address}_${asset}`] = await fetchAssetBalance(
    account,
    asset
  )
}

async function expectAssetBalanceChange(
  account: Account,
  asset: string,
  change: Number
) {
  ...
  expect(actual, `${account.address} balance of ${asset}`).to.be.eq(
    previous + change
  )
  ...
  it('init data [asset, device price, key price]', async () => {
    await txSuccess(
      Transactions.data(
        {
          data: [
            { key: 'asset', value: testsData.assetID },
  ...
  it('add oracles', async () => {
    await txSuccess(
      Transactions.data(
        {
          data: [
            { key: 'oracle_' + accountOracle.address, value: 'active' },
...
describe('addUser', () => {
  ...
  it('not by oracle', async () => {
    ...
    expect(
      await dappValueFor(accountDapp, `user_${accountDevOwner.address}`)
    ).to.not.eq(ACTIVE)
  })
...
describe('addDevice', () => {
  const addDeviceCall: IInvokeScriptCall = {
    function: 'addDevice',
    args: [{ type: 'string', value: accountDevice.address }]
  }
      ...
      Transactions.invokeScript(
        {
          ...
          call: addDeviceCall,
          payment: [
            { amount: testsData.devicePrice, assetId: testsData.assetID }
          ],
          fee: 900000
        },
        accountDummy.seed
  ...
  it(' wrong asset', async function () {
    await txFail(
      ...
      'wrong asset, supported only xyz'
    )
      ...
      Transactions.invokeScript(
        {
          ...
          call: addDeviceCall,
          payment: [
            { amount: testsData.devicePrice - 1, assetId: testsData.assetID }
          ],
          fee: 900000
        },
        accountDevOwner.seed
      ),
      'wrong payment value, expected ' + testsData.devicePrice
    )
  ...
  it('adds device', async function () {
    ...
    await expectAssetBalanceChange(
      accountDevOwner,
      testsData.assetID,
      -testsData.devicePrice
    )
  })
...
describe('addKey', () => {
      ...
      Transactions.invokeScript(
        {
          ...
          call: testsData.addKeyCall,
          payment: [
            { amount: testsData.keyPrice - 1, assetId: testsData.assetID }
          ],
          fee: 900000
        },
        accountDevOwner.seed
      ),
      'wrong payment value, expected ' + testsData.keyPrice
    )
  ...
  it('not owned token', async function () {
    await txFail(
      Transactions.invokeScript(
        {
          ...
          call: {
            ...
            args: [
              ...
              {
                type: 'string',
                value: 'FjTTApuujLAvQRudsgRfkj3qkWF9xmbfsATSt4ky5ELA'
              }
            ]
          },
          payment: [{ amount: testsData.keyPrice, assetId: testsData.assetID }],
      ...
      'key is not present in user wallet'
    )
      ...
      Transactions.invokeScript(
        {
          ...
          call: {
            function: 'removeKey',
            args: [{ type: 'string', value: testsData.userNftToRemove }]
          },
          payment: [],
          fee: 900000
        },
      ...
      Transactions.invokeScript(
        {
          ...
          call: {
            function: 'open',
            args: [{ type: 'string', value: testsData.userNft }]
          },
          payment: [],
          fee: 500000
        },
      ...
      Transactions.invokeScript(
        {
          ...
          call: {
            function: 'open',
            args: [{ type: 'string', value: testsData.assetID }]
          },
          payment: [],
          fee: 500000
        },
